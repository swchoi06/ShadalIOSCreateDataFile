...
-(BookHandle)excelBook{
    ...
    NSString * filename = [NSString stringWithFormat:@"SNU 전단지 관악캠퍼스"];
...
- (Restaurant *)getResFromExcelSheetIndex:(int)index{
    ...
    res.flyer = xlSheetReadNumA(sheet, 1, 6, NULL);
    res.coupon = xlSheetReadNumA(sheet, 2, 6, NULL);
    res.couponString = [NSString stringWithUTF8String:xlSheetReadStrA(sheet, 3, 6, NULL)];
